...
%  END OF LICENSE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cout, coef]=fracDerivativeSIM(dt,DataMatrix,alpha,coef,NumericalAlgorithm) 
%%%%%%%%% Numerical  Fract. Derivative of the Riemann-Liouville type %%%%%%
% L1 Oldham algorithm: 
%K.B. Oldham and J. Spanier
%The Fractional Calculus: Theory and applications of
% differentiation and integration to arbitrary order
%Dover, Mineola, NY 2002

% dt --> dt
% DataMatrix-the mxn matrix (double) to be fractionally differentiated. n is
% the number of time signals. m is the length of each
% signal, i.e. the number of time points in each signal.
% alpha --> fractional order alpha, 0<=alpha<1 
% coef=zeros(1,NSS); is the integration weight coefficients and NSS=Number of Simulation Steps
%NumericalAlgorithm -> 'L1' or 'L1_GPU'

% INPUT PARAMETER CHECKS
if ( alpha < 0 || alpha>=1) 
   display('This method works only for 0<=alpha<1');
   return;
end
...
mu=1-alpha;
MultC=(dt^(-alpha))/gamma(2-alpha);
             
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Handle < 2 time point case
% For NN =1, the result is always the same
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
switch NN
    case 1
        cout=zeros(1,cols);
        nn=1:NSS-1;
        coef=(NSS-nn).^mu-(NSS-nn-1).^mu;
    otherwise

switch NumericalAlgorithm
    ...
    %%%%%%%%%%%%%%%%%   L1 Oldham algorithm   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        WCoe0=mu/((NN-1)^alpha);
        WCoe=coef(end-NN+2:end);
        TeDi=DataMatrix(2:end,:)-DataMatrix(1:end-1,:);
        cout=MultC*(WCoe0*DataMatrix(1,:)+WCoe*TeDi);
    %%%%%%%%%%%%%%%%%%%%%% END CASE L1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
     if (  gpuDeviceCount == 0 )
         ...
         MultCGPU=gpuArray(MultC);
         WCoe0=gpuArray(mu/((NN-1)^alpha));
         DataMatrixGPU=gpuArray(DataMatrix);
         WCoeGPU=gpuArray(coef(end-NN+2:end));
         TeDiGPU=gpuArray(DataMatrix(2:end,:)-DataMatrix(1:end-1,:));
         J00=WCoeGPU*TeDiGPU;
         
         cout=gather(MultCGPU*(WCoe0*DataMatrixGPU(1,:)+J00));
     end
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
   otherwise
        warning('Only cases are L1 and L1_GPU')
end 
