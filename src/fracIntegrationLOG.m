...
%  END OF LICENSE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [iout,DataOut] = fracIntegrationLOG(t,dt,DataMatrix,alpha,DataOut)
    
    siz=size(DataMatrix); % rows, cols
    SL = siz(1);% length of the signals( number of rows )
    BSP  =DataOut{1};
    SwPts=DataOut{2};
    IDX  =DataOut{3};
    CG   =DataOut{4};
    ...
    if ( SL<SwPts(1) )
       iout = fracIntegration(dt,DataMatrix,alpha,'LO',1);
    end
    
    if( SL >= SwPts(1) )
        % CG tells us the latest Switch Point
        CG=CG+BSP(SL);
    
        % Create our current indices
        curIdx= IDX{CG};
        
        initIdx = [1:(SL-SwPts(CG))+1]';
        curIdx =  curIdx+(SL-SwPts(CG)); % New indices
        AllIdx=[initIdx;curIdx]';
        NN=length(AllIdx);
        ...
        a00=((tend-t2)^(1+alpha) +(t2*(1+alpha)-tend)*(tend^alpha))/t2;
        ajNN1=(tejm1.^(1+alpha)-(tej.^alpha).*( alpha*tjjm1+tejm1))./tjjm1;
        ajNN2=(tejp1.^(1+alpha)-(tej.^alpha).*( -alpha*tjp1j+tejp1))./tjp1j;
        
        ajNN=ajNN1+ajNN2;
        aNN=dt^alpha;
        acoef=[a00 ajNN aNN];
        
        tempData = DataMatrix(AllIdx,:);
        
        iout=C1*(acoef*tempData);
