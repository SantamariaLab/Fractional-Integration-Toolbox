...
%  END OF LICENSE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cout]=fracIntegration(dt,DataMatrix,alpha,TypeOfIntegration,numOMPThreads) 
%%%%% Numerical Calculation  of the Riemann-Liouville Fract.Integral %%%%%%
%%%%%%%%%%%%%%%%%%%%%%% 5 numerical routines %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1) LO- leading order. This case runs entirely in MATLAB and does not
% require compiling of frint.cpp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CUBIC, HERMITE, GPU_CUBIC and GPU_HERMITE require compiling of frint.cpp.
% To do so in Linux, run compileLinux.m in Matlab (normally gcc is the default)
% To do so in Windows, you run compileWindows.m(you have to have a Matlab
% compatible compiler in Windows)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2) CUBIC- leading order and first correction.
% 3) HERMITE- leading order and first correction
% 4) GPU_CUBIC- leading order and first correction
% 5) GPU_HERMITE- leading order and first correction
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



% fracIntegration(dt,DataMatrix,alpha,TypeOfIntegration,numOMPThreads);
% dt -> the time step (double), dt>0, for ex. 0.1, 0.5, etc
% DataMatrix-> the mxn matrix (double) to be fractionally integrated. n is
% the number of time signals to be integrated. m is the length of each
% signal, i.e. the number of time points in each signal.
% alpha ->  fractional order alpha (double), alpha>0
% TypeOfIntegration-> 'LO', 'CUBIC', 'HERMITE', 'GPU_CUBIC' or
% 'GPU_HERMITE' (string)
%numOMPThreads- number of OMP threads . Used only in CUBIC, HERMITE, GPU_CUBIC,
%GPU_HERMITE. In the case of LO parallelization is handled
%automaticallty by MATLAB. In that case the value of the parameter
%numOMPThreads is not used, however some value must be provided, for ex. 1.

%Example: For a given random DataMatrix of size (10,5) (5 signals with 10 data poits each)
%DataMatrix=rand(10,5)
%you can call the fracIntegration routine 
%fracIntegration(0.1,DataMatrix,0.5,'HERMITE',2)

%The numerical method we use is described in "Algorithms for the Fractional Calculus:
% A Selection of Numerical Methods", K. Diethlem, N.J. Ford, A.D. Dreed,
% Yu. Luchko, Comp.Methods Appl. Mech. Engrg. 194(2005)

%The fractional Riemann-Liouville integral is always evaluated for
%integration limits 0 and T=(m-1)*dt

% INPUT PARAMETER CHECKS
if ( alpha <= 0 ) 
   display('Alpha must be positive');
   return;
end

% SETUP FOR ALL TYPES OF INTEGRATION
r0=2; % always
% if( alpha <= 1 )
% r1=2+alpha; % only used for second corrections (maybe future work)
% else
% r1 = 3;
% end

siz=size(DataMatrix); % rows, cols
NN = siz(1);    % length of the signals( number of rows )
cols = siz(2);

             
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Handle NN=1, NN=2 and NN>2 time point cases.
% For NN=1 and NN=2 the result is the same for all integration routines
% For NN =1 , the result is always zero.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
switch NN
    case 1
        cout=zeros(1,cols);
    case 2
        co00=[alpha,1];
        cout=((dt^alpha)/gamma(2+alpha))*(co00*DataMatrix);
...
switch TypeOfIntegration
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        nn=1:NN-2;
        co00=[(1+alpha)*(NN-1)^alpha-(NN-1)^(1+alpha)+(NN-2)^(1+alpha), (NN-nn).^(1+alpha)-2*(NN-nn-1).^(1+alpha)+(NN-nn-2).^(1+alpha), 1];
        cout=((dt^alpha)/gamma(2+alpha))*(co00*DataMatrix);

    %%%%%%%%%%%%%%%%%%%%%% END CASE LO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
              
     %%%%%%%%%%%%%%%  Leading Order + First Correction %%%%%%%%%%%%%%%%%%%%          
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
     case 'CUBIC'
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
     c1 = (-1/3) * ( (dt^alpha) / ( gamma(2+alpha))  );
     c2 = (-4/(2^alpha))*c1; 
     cout =frint_cubic_double(DataMatrix,dt,alpha,c1,c2,numOMPThreads);
     %%%%%%%%%%%%%%%%%%%%%%% END CASE CUBIC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                       
     ...
     c1 = (-1/3) * ( (dt^alpha) / ( gamma(2+alpha))  );
     c2 = (-4/(2^alpha))*c1; 
     cout =frint_hermite_double(DataMatrix,dt,alpha,c1,c2,numOMPThreads);
     %%%%%%%%%%%%%%%%%%%%%%% END CASE HERMITE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
     if (  gpuDeviceCount == 0 )
         warning('No GPU devices found in your system');
     else
         DataMatrixGPU=gpuArray(DataMatrix);
         nn=1:NN-2;
         co00=gpuArray([(1+alpha)*(NN-1)^alpha-(NN-1)^(1+alpha)+(NN-2)^(1+alpha), (NN-nn).^(1+alpha)-2*(NN-nn-1).^(1+alpha)+(NN-nn-2).^(1+alpha), 1]);
         J00=(((dt^alpha)/gamma(2+alpha))*(co00*DataMatrixGPU));
         NN10=2*NN-1;
         nn10=1:NN10-2;
         c1 =0;
         c2 =0; 
         y10SPLINE =interpolate_cubic_double(DataMatrix,dt,alpha,c1,c2,numOMPThreads);
         y10GPU = gpuArray(y10SPLINE);
         co10=gpuArray([(1+alpha)*(NN10-1)^alpha-(NN10-1)^(1+alpha)+(NN10-2)^(1+alpha), (NN10-nn10).^(1+alpha)-2*(NN10-nn10-1).^(1+alpha)+(NN10-nn10-2).^(1+alpha), 1]);
         J10=(((dt/2)^alpha)/gamma(2+alpha))*(co10*y10GPU);
         cout=(gather((J00-(2^r0)*J10)/(1-2^r0)));
     end
     %%%%%%%%%%%%%%%%%%%%%%% END CASE GPU_CUBIC %%%%%%%%%%%%%%%%%%%%%%%%%%% 
     ...
     if (  gpuDeviceCount == 0 )
         ...
         c1 =0;
         c2 =0; 
         y10SPLINE =interpolate_hermite_double(DataMatrix,dt,alpha,c1,c2,numOMPThreads);
         y10GPU = gpuArray(y10SPLINE);
         co10=gpuArray([(1+alpha)*(NN10-1)^alpha-(NN10-1)^(1+alpha)+(NN10-2)^(1+alpha), (NN10-nn10).^(1+alpha)-2*(NN10-nn10-1).^(1+alpha)+(NN10-nn10-2).^(1+alpha), 1]);
         J10=(((dt/2)^alpha)/gamma(2+alpha))*(co10*y10GPU);
         cout=(gather((J00-(2^r0)*J10)/(1-2^r0)));
     end
     %%%%%%%%%%%%%%%%%%%%%%% END CASE GPU_HERMITE %%%%%%%%%%%%%%%%%%%%%%%%%%% 


%      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%      case 'SPLINE_MATLAB' %This routine is functional, but very slow&inefficient as the number of signals goes up 
%      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%             display('This redefines slow and painful')
%             UU=t(1):(dt/2):t(end); % support points, the knots( the time atis )
%             pp=zeros(length(UU),cols); % contains space for interpolated data
%             parfor i=1:cols
%                 pp(:,i) = spline(t,DataMatrix(:,i),UU);
%                 %pp(:,i) = pchip(t,DataMatrix(:,i),UU);
%             end
%             %%%%%%%%%%%%%%%%%Zero Order%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             nn=2:NN-1;
%             co00=[(1+alpha)*NN^alpha-NN^(1+alpha)+(NN-1)^(1+alpha),(NN-nn+1).^(1+alpha)-2*(NN-nn).^(1+alpha)+(NN-nn-1).^(1+alpha),1];
%             J00=((dt^alpha)/gamma(2+alpha))*(co00*DataMatrix);
%             %%%%%%%%%%%%%%%%%First Correction%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             NN10=2*NN-1; % The new number of signal points
%             nn10=2:NN10-1;
%             co10=[(1+alpha)*NN10^alpha-NN10^(1+alpha)+(NN10-1)^(1+alpha),(NN10-nn10+1).^(1+alpha)-2*(NN10-nn10).^(1+alpha)+(NN10-nn10-1).^(1+alpha),1]; 
%             J10=(((dt/2)^alpha)/gamma(2+alpha))*(co10*pp);
%             cout=((J00-(2^r0)*J10)/(1-2^r0))';
%         %%%%%%%%%%%%%%%%%%%%%%END CASE Spline_Matlab %%%%%%%%%%%%%%%%%%%%%%
 

    otherwise
       warning('Invalid Integration Type. Please use a valid integration type.')
       warning('Use LO, CUBIC, HERMITE, GPU_CUBIC or GPU_HERMITE')  
end  
end 
end
