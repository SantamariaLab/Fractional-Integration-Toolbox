...
%  END OF LICENSE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cout,coef]=fracIntegrationSIM(dt,DataMatrix,alpha,coef,TypeOfIntegration) 
%%%%% Riemann-Liouville Fract.Integral in a Simulation Mode %%%%%%
% We use the same numerical method as in fracIntegration LO, however the
% calculation of the integration weights is optimized.


% fracIntegrationSIM(dt,DataMatrix,alpha,coef,TypeOfIntegration);
...
% coef=zeros(1,NSS); is the integration weight coefficients and NSS=Number of Simulation Steps
% TypeOfIntegration->LO

%For example, in a simulation  calling 
%fracIntegrationSIM(dt,DataMatrix,alpha,coef,'LO')
%looks like:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% alpha=0.5;
% Tmax=10;
...
% coef=zeros(1,NSS);
% NumberOfSignals=20;
% DataMatrix=rand(1,NumberOfSignals);
% for i=1:NSS
%   [cout,coef]=fracIntegrationSIM(dt,DataMatrix,alpha,coef,'LO');  
%   DataMatrix=[DataMatrix; cout+0.5];
%   i
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Tmax=length(coef);           
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Handle < 3 time point cases as well as => 3 cases.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
switch NN
    case 1
        cout=zeros(1,cols);
    ...
    case 2
        coeft=[alpha,1];
        cout=((dt^alpha)/gamma(2+alpha))*(coeft*DataMatrix);
    otherwise
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    case 'LO'
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
  coef(Tmax-NN+2)=(NN-1)^(1+alpha)-2*(NN-2)^(1+alpha)+(NN-3)^(1+alpha);
  coef(Tmax-NN+1)=(1+alpha)*(NN-1)^alpha-(NN-1)^(1+alpha)+(NN-2)^(1+alpha);
       
  cout=((dt^alpha)/gamma(2+alpha))*(coef(Tmax-NN+1:end)*DataMatrix);
%%%%%%%%%%%%%%%%%%%%%%%%%%%    END CASE LO  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 if (  gpuDeviceCount == 0 )
  ...
  coef(Tmax-NN+1)=(1+alpha)*(NN-1)^alpha-(NN-1)^(1+alpha)+(NN-2)^(1+alpha);
         ...
         MultCGPU=gpuArray((dt^alpha)/gamma(2+alpha));
         coefGPU=gpuArray(coef(Tmax-NN+1:end));
         DataMatrixGPU=gpuArray(DataMatrix);
         
         cout=gather(MultCGPU*(coefGPU*DataMatrixGPU));
  end
%%%%%%%%%%%%%%%%%%%%%%%%%%%   END CASE LO_GPU %%%%%%%%%%%%%%%%%%%%%%%%%%%%%

otherwise
        warning('Only case is LO and LO_GPU')
end 
end 
end
