...
%  END OF LICENSE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [DataOut]=LogSimIDXWTS(NT,IL,q)
...
tv=[]; 
%wts=[];
for i=1:length(SwPts)
    tv=[];
    %wts=[];
    for j=i:-1:1
        tv1=[((SwPts(i)-IL*q^(j+1))+1):q^j:(SwPts(i)-IL*q^(j))]';
        %wts1=(q^(j*alpha))*ones(size(tv1));
        tv=[tv;tv1];
        %wts=[wts;wts1];
    end
    temp=(tv(end)+1:SwPts(i))';
    %lind=ones(length(temp),1);
    IDX{i}=[tv;temp];
    %WTS{i}=[wts;lind];
